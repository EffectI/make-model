...
OUTPUT_ROOT_DIR = 'make-model/model/result_models/koelectra_small_lora' # 경로 이름 변경 권장
...
def run_kfold_process():
    ...
    print(f"\n[{MODEL_NAME}] {N_FOLDS}-Fold LoRA 학습 시작")
    ...
    for fold_idx in range(N_FOLDS):
        print(f"\n" + "="*40)
        print(f" >>> [FOLD {fold_idx}] Start Training (with LoRA)")
        print("="*40)
        ...
        # 2. [변경됨] LoRA Config 설정
        # KoELECTRA는 BERT/Electra 구조이므로 'query', 'value' 모듈을 타겟으로 잡습니다.
        peft_config = LoraConfig(
            ...
            target_modules=["query", "value"] # Attention 메커니즘의 Query, Value Projection에 적용
        )

        # 3. [변경됨] 모델을 LoRA 모델로 래핑
        model = get_peft_model(model, peft_config)
        
        # 학습 가능한 파라미터 수 출력 (LoRA가 잘 적용되었는지 확인용)
        print(">>> LoRA 적용 후 Trainable Parameters 확인:")
        ...
        args = TrainingArguments(
            ...
            eval_strategy="epoch",
            save_strategy="epoch",
            learning_rate=LEARNING_RATE, # LoRA는 보통 Base보다 높은 LR 사용
    ...
    # ==========================
    print("\n" + "#"*50)
    print(" [K-Fold Training Summary (with LoRA)]")
    print("#"*50)
